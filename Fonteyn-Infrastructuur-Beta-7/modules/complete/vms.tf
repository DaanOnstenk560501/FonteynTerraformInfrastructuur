...
resource "azurerm_windows_virtual_machine" "servers" {
  ...
  # --- FIX 1: Add computer_name attribute (max 15 chars) ---
  computer_name         = substr("vm-${each.key}", 0, 15) # Dynamically truncate name to 15 chars
  ...
  boot_diagnostics {
    # --- FIX 2: Use primary_blob_endpoint for boot diagnostics URI ---
